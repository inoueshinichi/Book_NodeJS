...
const eventBEventEmitter = new events.EventEmitter()

const eventBPromise = events.once(eventBEventEmitter, 'eventB')

// Promiseでイベントemitを処理
eventBPromise.then(arg => console.log('eventB発生', arg))


eventBEventEmitter.emit('eventB', 'Hello', 'World')
eventBEventEmitter.listeners()

// 一度でもeventBが発行されたらeventBPromiseはfulfilledになるため、以降のイベント発行には反応しない
eventBEventEmitter.emit('eventB', 'one more')
