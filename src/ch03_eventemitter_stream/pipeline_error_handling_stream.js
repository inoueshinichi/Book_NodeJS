...
// ストリームのエラーハンドリングで注意すべきは、
// piple()がエラーを伝播させないこと.
// 自分で`error`イベントに対してリスナ登録が必要
const SWITCH = 4

if (SWITCH === 1) {
    // bad
fs.createReadStream('./data/no-such-file.txt')
    .pipe(fs.createWriteStream('./data/virtual_dst.txt'))
...
if (SWITCH === 2) {
    // good
fs.createReadStream('./data/no-such-file.txt')
    .on('error', err => console.error('[Read] エラーイベント', err.message))
    .pipe(fs.createWriteStream('./data/virtual_dst.txt'))
    .on('error', err => console.error('[Write] エラーイベント', err.message))
}
...
if (SWITCH === 3) {
    stream.pipeline(
        // pipe()したい2つ以上のストリーム
        fs.createReadStream('./data/no-such-file.txt'),
        fs.createWriteStream('./data/virtual_dst.txt'),
        // コールバック
        (err) => {
            if (err) {
                console.error('Pipelineでエラー発生', err.message)
            } else {
                console.info('正常終了')
            }
        }
...
if (SWITCH === 4) {
    (async () => {
        ...
        try {
            ...
            console.info('pipelineが正常終了しました。')
        } catch (err) {
            console.error('Pipelineでエラー発生', err.message)
        }
    })()
